...
CREATE TABLE [dbo].[VehiclePartnerProfiles] (
    ...
    [PermanentProvinceCode] varchar(2) NULL,
    [PermanentWardCode] varchar(5) NULL,
    ...
    [CurrentProvinceCode] varchar(2) NULL,
    [CurrentWardCode] varchar(5) NULL,
    ...
    [HeadquartersProvinceCode] varchar(2) NULL,
    [HeadquartersWardCode] varchar(5) NULL,
...
BEGIN TRY
    ...
    IF (SELECT COUNT(*) FROM #ProvinceSeed) <> 34
        THROW 53123, N'Danh mục hành chính phải có đúng 34 tỉnh/thành phố.', 1;
    IF (SELECT COUNT(*) FROM #WardSeed) <> 3321
        THROW 53124, N'Danh mục hành chính phải có đúng 3321 xã/phường/đặc khu.', 1;
    ...
    IF (SELECT COUNT(*) FROM dbo.AdministrativeProvinces WHERE IsActive=1) <> 34
        THROW 53125, N'Sau khi cài đặt phải có đúng 34 tỉnh/thành phố hoạt động.', 1;
    IF (SELECT COUNT(*) FROM dbo.AdministrativeWards WHERE IsActive=1) <> 3321
        THROW 53126, N'Sau khi cài đặt phải có đúng 3321 xã/phường/đặc khu hoạt động.', 1;

    UPDATE dbo.SystemVersions SET IsCurrent=0 WHERE IsCurrent=1;
    IF EXISTS (SELECT 1 FROM dbo.SystemVersions WHERE DatabaseVersion=N'31.0')
        UPDATE dbo.SystemVersions
           SET ApplicationVersion=N'31.0.16', ReleasedDate=SYSUTCDATETIME(), IsCurrent=1,
               Notes=N'v31.0.16: hồ sơ đối tác dùng đủ 34 tỉnh và 3321 xã/phường/đặc khu; lưu mã hành chính; khóa đăng nhập 5 lần trong 15 phút.'
         WHERE DatabaseVersion=N'31.0';
    ELSE
        INSERT dbo.SystemVersions(ApplicationVersion,DatabaseVersion,ReleasedDate,IsCurrent,Notes)
        VALUES(N'31.0.16',N'31.0',SYSUTCDATETIME(),1,N'v31.0.16: hồ sơ đối tác dùng đủ 34 tỉnh và 3321 xã/phường/đặc khu; lưu mã hành chính; khóa đăng nhập 5 lần trong 15 phút.');
...
/* ============================================================================
   v31.0.16 - ĐỒNG BỘ 4 TÀI KHOẢN DEMO GỐC
